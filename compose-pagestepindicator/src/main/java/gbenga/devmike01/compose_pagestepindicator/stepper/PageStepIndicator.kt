...
private fun DrawPageStepIndicator(
    ...
    val animateStep = remember{
        Animatable(0f)
    }


    LaunchedEffect(key1 =circleRadius){
        animateStep.animateTo(1f,
    ...
    Canvas(
       ...
       // val mLabels = labels.subList(1, labels.size)
        ...
        drawIntoCanvas {
            ...
            val startX = (lineWidth + circleRadius) * (labels.size-1)

            val centerIndicatorX = (canvasWidth -startX).div(2)
            ...
            labels.forEachIndexed{ i, label ->
                ...
                drawAndPaintSteps(
                    ...
                    stepPaint,
                    i, labelTopSpace,
                    selectedPosition, stepBorderPaint,
                    stepCountPaint, onStepClick,
                    stepCoords,
                    circleRadius,
                    labels,
                    pathHeight,
...
fun drawAndPaintSteps(
    animateStep: Animatable<Float, AnimationVector1D>,
    centerIndicatorX: Float,
    ...
    val circleOffset =  if (i ==0){
        Offset(x = centerIndicatorX, y = (canvasHeight /1.5f))
    }else{

        Offset(
            x = (((lineWidth) * i) + centerIndicatorX)
                    * (animateStep.value.takeIf { it >0f } ?: 1f),
            y = (canvasHeight / 1.5f)
        )
    }
...
fun calculatePosition(coords: List<Offset>,
                      coordX: Float,
                      coordY: Float,
                      radius: Float): Int {

    val firstCord = coords[1].x
    ...
    for(element in coords){
        ...
        Log.d("calculatePosition", "$yClicked _ $xClicked")

        if((yClicked).and(xClicked)){
            val position = ((coordX / firstCord))
                .nextUp();
